...
DEVICE_TREE="${BSP_PATH}/tools/flash_tool/chips/bl602/device_tree/bl_factory_params_IoTKitA_40M.dts"
...
if [ "${3}" = "" ]; then
    make CONFIG_CHIP_NAME=BL602 CONFIG_LINK_ROM=1 CONFIG_BLE_TP_SERVER=1 CONFIG_BLECONTROLLER_LIB=all -j PTS_GAP_SLAVER_CONFIG_INDICATE_CHARC=1
elif [ "${3}" = "clean" ]; then
    rm -rf ${BUILD_OUTPUT_PATH}
elif [ "${3}" = "erase_flash" ]; then
    "${BL_FLASH_TOOL}" --chipname="${CHIP_NAME}" --baudrate="${BAUDRATE}" --port="${4}" --pt="${PARTITION_TABLE}" --dts="${DEVICE_TREE}" --firmware="${APP_FIRMWARE_PATH}" --erase
elif [ "${3}" = "flash" ]; then
    if [ ! -d "${BUILD_OUTPUT_PATH}" ]; then
        make CONFIG_CHIP_NAME=BL602 CONFIG_LINK_ROM=1 CONFIG_BLE_TP_SERVER=1 CONFIG_BLECONTROLLER_LIB=all -j PTS_GAP_SLAVER_CONFIG_INDICATE_CHARC=1
    fi
    "${BL_FLASH_TOOL}" --chipname="${CHIP_NAME}" --baudrate="${BAUDRATE}" --port="${4}" --pt="${PARTITION_TABLE}" --dts="${DEVICE_TREE}" --firmware="${APP_FIRMWARE_PATH}"
elif [ "${3}" = "monitor" ]; then
