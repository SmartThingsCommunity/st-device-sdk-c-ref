...
static void button_event(IOT_CAP_HANDLE *handle, int type, int count)
{
	if (type == BUTTON_SHORT_PRESS) {
		...
		/* clean-up provisioning & registered data */
		printf("self cleanup & trigger connection again\n");
		...
		iot_stat_lv_t stat_lv, void *usr_data)
{
	...
	switch(status)
	{
		case IOT_STATUS_PROVISIONING:
			/* If you want to re-trigger on-boarding procedure automatically
			 * on the on-boarding waiting timeout case,	enable follwoings
			 *	if (stat_lv == IOT_STAT_LV_FAIL) {
			 *		printf("Timeout faild, trigger connection again\n");
			 *		trigger_st_conn = 1;
			 *	}
			 */
			break;
		case IOT_STATUS_NEED_INTERACT:
			...
			/* If you want to re-trigger on-boarding procedure automatically
			 * on the key-confirm timeout case,	enable follwoings
			 *	if (stat_lv == IOT_STAT_LV_FAIL) {
			 *		printf("Button confirm faild, trigger connection again\n");
			 *		trigger_st_conn = 1;
			 *	}
			 */
			break;
...
static void onboarding_task(void *arg)
{
	for (;;) {
		if (trigger_st_conn) {
			...
			// 1. to decide the pin confirmation number(ex. "12345678"). It will use for easysetup.
			//    pin confirmation number must be 8 digit number.
			pin_num_memcpy(pin_num, "12345678", sizeof(iot_pin_t));

			// 2. process on-boarding procedure. There is nothing more to do on the app side than call the API.
			st_conn_start(ctx, (st_status_cb)&iot_status_cb, IOT_STATUS_ALL, NULL, pin_num);

			free(pin_num);
#else
			// 1. process on-boarding procedure. There is nothing more to do on the app side than call the API.
...
void app_main(void)
{
	/**
	  ...
	  //needed for on-boarding procedure to control it by the button event.
	  5. onboarding_task()
	 */
	...
	// 5. needed for on-boarding procedure to control it by the button event.
	//  quickly button press twice will be trigger the on-boarding procedue
	xTaskCreate(onboarding_task, "onboarding_task", 2048, NULL, 10, NULL);
}
